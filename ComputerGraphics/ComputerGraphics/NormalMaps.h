...
public:
	...
	void init() {
		...
		auto meGeo =  addGeometry(Neumont::ShapeGenerator::makePlane(10),GL_TRIANGLES);
		...
		ShaderProgram * meShader = addShader("./../shaders/basicNormalV.glsl","./../shaders/basicNormalF.glsl"); // for clock, clock face
		saveViewTransform(meShader,"viewTransform");
		meShader->saveUniform("lightSrc",   ParameterType::PT_VEC3, &lightSrc[0]       );
		...
		mePlane->whereMat = glm::rotate(180.0f,glm::vec3(1,0,0));



		menu->edit("lightPosition",lightSrc,1,200);
		menu->edit("lightColor",specPower,1,200);
